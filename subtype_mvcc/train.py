...
def prepare_trte_data(file_input): 
    l = [] 
    data_tr_list = []
    
    for file in file_input: 
        df_new = pd.read_csv(file, sep=',', header=0, index_col=0)    
        l = list(df_new)             
        df_new = df_new.T
...
def gen_trte_adj_mat(data_tr_list,num_class=6):
    ...
    adj_train_list = []
    for i in range(len(data_tr_list)): 
        adj_train_list.append(gen_adj_mat_tensor(data_tr_list[i], num_class, adj_metric)) 
...
def train_test(file_input, output_folder, view_list, num_class,lr_e,total_epochs):
    num_view = len(view_list)           
    dim_he_list = [300,200,100]  # the output dimensions of 3 GCN-layer       
    l=[]
    ############## prepare_trte_data ###################
    data_tr_list,l = prepare_trte_data(file_input) 
    ...
    for key in loss_list_dict:
        ...
        # plt.show()    
        plt.savefig(output_folder + '/{}_show.png'.format(key))
